...
<x-app-layout>
  {{-- Don’t rely on Jetstream header slot --}}
  <x-slot name="header"></x-slot>

  {{-- Visible page title bar (make sticky by keeping the sticky classes) --}}
  <div class="bg-white/95 backdrop-blur sticky top-0 z-20 border-b">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
      <h1 class="font-extrabold text-2xl sm:text-3xl text-gray-900">Wishlist</h1>
    </div>
  </div>
        ...
        @else
          <section class="mt-2"
            x-data="{
              ...
              init(){
                this.$nextTick(()=>{
                  const first = this.$refs.scroller?.querySelector('[data-card]');
                  const gap = 24;
                  this.step = (first ? first.offsetWidth : 227) + gap;
                  this.onScroll();
                });
              },
              onScroll(){
            ...
            <div class="relative">
                    ...
                    <div data-card data-wish-id="{{ $it->id }}" class="snap-start shrink-0 block group" style="width:6cm">
                      <a href="{{ route('user.product.preview', $p) }}" class="block">
                        ...
                        <div class="pt-3">
                          ...
                          <div class="mt-1 font-semibold text-[16px] leading-snug line-clamp-1">
                            {{ $p->product_name }}
                          </div>
                          <div class="mt-1 text-gray-600 text-[15px]">
                            {{ $p->category }}
                          </div>
                        </div>
                      </a>
              ...
              <button
                @click="prev" x-show="!atStart" x-cloak
              ...
              <button
                @click="next" x-show="!atEnd" x-cloak
