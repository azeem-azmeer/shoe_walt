...
<x-app-layout>
  ...
  <main id="main-content" class="bg-white">
    ...
    {{-- MINI CART POPUP --}}
    <div x-data="$store.miniCart" x-show="open" x-cloak class="fixed inset-0 z-50">
      ...
      <div class="absolute right-4 top-16 w-[360px] bg-white rounded-xl shadow-xl ring-1 ring-black/10">
        <div class="flex items-center justify-between px-4 py-3 border-b">
          ...
          <button class="h-8 w-8 rounded-full hover:bg-gray-100" @click="open=false">✕</button>
        </div>
          ...
          <template x-for="it in items" :key="it.id">
            <div class="flex gap-3 px-4 py-3">
              ...
              <div class="flex-1">
                ...
                <div class="text-sm font-bold" x-text="currency(it.price)"></div>
              </div>
              <button class="text-xs text-red-600 hover:underline" @click="remove(it.id)">Remove</button>
            </div>
          </template>
        ...
        <div class="px-4 py-3 border-t flex items-center justify-between">
          {{-- View Bag always goes to cart page --}}
          <a href="{{ route('user.viewbag') }}"
            class="px-3 py-2 rounded border hover:bg-gray-50 text-sm">
            View Bag
          </a>

          {{-- Checkout: send logged-in users to /checkout, guests to register --}}
          @auth
            <a href="{{ route('user.checkout') }}"
              class="px-3 py-2 rounded bg-black text-white text-sm font-semibold">
              Checkout
            </a>
          @else
  ...
  <script>
    (function(){
      ...
      if (window.Alpine && Alpine.store) { try{register();}catch(e){} }
      document.addEventListener('alpine:init', ()=>{ try{register();}catch(e){} });
    })();
  </script>

  <script>
    document.addEventListener('alpine:init', () => {
      // Strong, always-defined constants (no 'undefined' redirect)
      const IS_AUTH = @json(auth()->check());
      const REG_URL = @js(route('register')); // server-resolved
      const REG_FALLBACK = '/register';
      ...
      Alpine.store('miniCart', {
        ...
        openWith(items, count){ this.items=items||[]; this.count=count||0; this.open=true; const badge=document.getElementById('cart-count'); if(badge) badge.textContent=String(this.count); },
        async remove(id){
          await ensureCsrf();
          const res = await fetch(`/api/cart/${id}`, { method:'DELETE', headers:{ 'X-CSRF-TOKEN': csrfToken() }, credentials:'same-origin' });
          if (res.status===401){ window.location.assign(REG_URL || REG_FALLBACK); return; }
          if (!res.ok){ Alpine.store('flash').show('Could not remove item.','error'); return; }
          const data = await res.json(); this.openWith(data.items, data.count);
      ...
      Alpine.store('actions', {
        ensureAuth(){
          ...
          // Always a valid path (no 'undefined')
          window.location.assign(REG_URL || REG_FALLBACK);
          return false;
        },
        ...
        async addToCart(productId){
          ...
          if (res.status===401){ window.location.assign(REG_URL || REG_FALLBACK); return false; }
          if (res.status===422){
