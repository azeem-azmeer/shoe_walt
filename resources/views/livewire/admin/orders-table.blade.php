    ...
    <div class="rounded-2xl bg-white/80 backdrop-blur p-5">
      <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-5">
        <div>
          ...
          <p class="text-sm text-gray-500 mt-1">Showing latest orders placed by all customers.</p>
        </div>
        ...
        <div class="flex flex-wrap items-end gap-3">
          ...
          <div>
            ...
            <select wire:model.live="status" class="rounded-xl border px-3 py-2">
          ...
          <div>
            ...
            <select wire:model.live="perPage" class="rounded-xl border px-3 py-2">
    ...
    <div class="bg-white border rounded-2xl shadow overflow-hidden">
      <div class="overflow-x-auto">
        <table class="min-w-full text-sm">
          <thead class="bg-gray-50/80 backdrop-blur text-gray-600 uppercase text-xs sticky top-0 z-10">
            <tr>
              ...
              <tr class="hover:bg-gray-50 transition" wire:key="order-{{ $order->id }}">
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center gap-3">
                    @php
                      $initial = mb_strtoupper(mb_substr($order->user->name ?? 'U', 0, 1));
                    @endphp
                    <div class="h-8 w-8 rounded-full bg-indigo-600/10 text-indigo-700 text-sm font-bold grid place-items-center">
                      {{ $initial }}
                    </div>
                    <div class="min-w-0">
                      <div class="font-semibold truncate max-w-[180px]">{{ $order->user->name ?? '—' }}</div>
                      <div class="text-[11px] text-gray-400 truncate max-w-[220px]">{{ $order->user->email ?? '—' }}</div>
                    </div>
                  </div>
                </td>
